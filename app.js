// ======================================================
// ===== CONFIGURACIÓN DE FIREBASE (Copiada al JS) =====
// ======================================================
...
tailwind.config = {
    theme: {
        ...
        // ======================================================
        // ===== VARIABLES GLOBALES (Paso 1: Refactorización) =====
        // ======================================================
        ...
        let usuarioActual = null; // ¡Clave para saber QUIÉN usa la app!
        const db_firestore = firebase.firestore(); // Conexión a la base de datos
        
        // --- Mapas de Datos de Firebase (Caché en tiempo real) (NUEVO) ---
        let firebaseAssignmentsMap = new Map();
        let firebaseHistoryMap = new Map();
        let firebaseChildOrdersMap = new Map();
        let firebaseDesignersMap = new Map(); // Se usará para 'designerList'
        let firebaseWeeklyPlanMap = new Map();
        
        // --- Variables de Lista y Estado ---
        let designerList = []; // Ahora se llenará desde firebaseDesignersMap
        const CUSTOM_STATUS_OPTIONS = ['Bandeja', 'Producción', 'Auditoría', 'Completada'];
        let needsRecalculation = true; 

        // --- Instancias de Gráficos (Original) ---
        ...
        // ======================================================
        // ===== FUNCIONES DE INICIALIZACIÓN (Paso 2: Firebase) =====
        // ======================================================
        
        document.addEventListener('DOMContentLoaded', (event) => {
            console.log('DOM cargado. Inicializando App v5.0 (Firebase)...');
            
            // --- Listeners de Autenticación (NUEVO) ---
            ...
            // --- Listener de Autenticación Principal (NUEVO) ---
            firebase.auth().onAuthStateChanged((user) => {
                ...
                if (user) {
                    ...
                    if (!isExcelLoaded) {
                        ...
                        dashboard.style.display = 'block'; // O mostrar dashboard si ya cargó
                    }
                    ...
                    // Aquí deberías desconectar los listeners de onSnapshot si existen
                    // (Por simplicidad, omitido, pero en producción se deben desconectar)
                }
            });
            ...
            // --- Listeners de Delegación (Originales) ---
        ...
        // ======================================================
        // ===== FUNCIONES DE FIREBASE (Paso 3: Núcleo) =====
        // ======================================================
        ...
        function conectarDatosDeFirebase() {
            ...
            db_firestore.collection('assignments').onSnapshot((snapshot) => {
                ...
                if(isExcelLoaded) mergeYActualizar(); // Refrescar si el Excel ya está cargado
            ...
            db_firestore.collection('history').onSnapshot((snapshot) => {
                ...
                console.log(`Sincronizados ${firebaseHistoryMap.size} historiales de órdenes.`);
            ...
            db_firestore.collection('childOrders').onSnapshot((snapshot) => {
                ...
                console.log(`Sincronizadas ${firebaseChildOrdersMap.size} órdenes con hijas.`);
            ...
            db_firestore.collection('designers').orderBy('name').onSnapshot((snapshot) => {
                ...
                console.log(`Sincronizados ${designerList.length} diseñadores.`);
            ...
            db_firestore.collection('weeklyPlan').onSnapshot((snapshot) => {
                ...
                // Refrescar el plan si está abierto
        ...
        function mergeYActualizar() {
            if (!isExcelLoaded) return; // No hacer nada si el Excel no está cargado
            ...
            for (let i = 0; i < allOrders.length; i++) {
                ...
                if (fbData) {
                    ...
                    // Si no existe en Firebase, asegurar que esté limpio
                    order.designer = '';
                    order.customStatus = '';
                    order.receivedDate = '';
                    order.notes = '';
                ...
                // Lógica de completado automático (igual que antes)
                if (fbData && 
         ...
         * Esta función REEMPLAZA a saveAssignmentToDB y saveToHistory.
         */
        async function saveAssignmentToDB_Firestore(orderId, dataToSave, historyChanges = []) {
            ...
            if (historyChanges.length > 0) {
                ...
                for (const change of historyChanges) {
                    const historyRef = db_firestore.collection('history').doc(); // ID automático
        ...
        /**
         * (Refactorizado) Agrega un diseñador a Firestore
         */
        async function addDesigner() {
            ...
            try {
                // Añadir a la colección 'designers'
        ...
        /**
         * (Refactorizado) Elimina un diseñador de Firestore
         */
        async function deleteDesigner(docId, name) {
            ...
            if (assignedOrders.length > 0) {
                message = `¿Estás seguro de eliminar a "${name}"? \n\n⚠️ ADVERTENCIA: Este diseñador tiene ${assignedOrders.length} orden(es) activa(s) en P_Art. Si continúas, estas órdenes quedarán "Sin asignar" en Firebase.`;
            }

            showConfirmModal(message, async () => {
                try {
                    // 1. Eliminar de la colección 'designers'
                    await db_firestore.collection('designers').doc(docId).delete();
                    
                    // 2. Desasignar órdenes en Firebase
                    const batch = db_firestore.batch();
                    ...
                    // onSnapshot se encargará de actualizar la UI
                    showCustomAlert(`Diseñador "${name}" eliminado. ${assignedOrders.length} órdenes fueron desasignadas.`, 'success');
        ...
        // ======================================================
        // ===== FUNCIONES BÁSICAS (Originales y Auxiliares) =====
        // ======================================================
        ...
        // ======================================================
        // ===== LÓGICA DE MANEJO DE EXCEL (Refactorizado) =====
        // ======================================================
        ...
        async function processFile(file) {
            ...
            try {
                ...
                for (const row of rows) {
                    ...
                    // --- (NUEVO) Fusión con datos de Firebase ---
                    const fbData = firebaseAssignmentsMap.get(orderId);
                    ...
                    const order = {
                        ...
                        childPieces: 0, // Se calculará después
                        isLate,
                        daysLate,
                        isVeryLate,
                        isAboutToExpire,
                ...
                isExcelLoaded = true; // Marcar que el Excel ya se cargó
                console.log(`✅ Órdenes procesadas del Excel: ${allOrders.length}`);

                needsRecalculation = true; 
                recalculateChildPieces(); // Construye el caché de hijas
        ...
        // ======================================================
        // ===== LÓGICA DE ÓRDENES HIJAS (Refactorizado) =====
        // ======================================================
        ...
        async function saveChildOrder() {
            try {
                ...
                const childOrder = {
                    childOrderId: `${parentOrder.orderId}_child_${Date.now()}`, // ID único para Firestore
                ...
                // onSnapshot se encargará de actualizar los mapas y la UI
                closeAddChildModal();
                showCustomAlert(`Orden hija ${childCode} creada exitosamente`, 'success');
                // No es necesario llamar a updateDashboard(), onSnapshot lo hará.
        ...
        async function deleteChildOrder(childOrderId, childCode) {
            showConfirmModal(`¿Estás seguro de eliminar la orden hija ${childCode}?`, async () => {
                try {
                    ...
                    // onSnapshot se encargará de actualizar todo
        ...
        /**
         * (Refactorizado) Carga órdenes hijas desde el Mapa de Firebase
         */
        ...
        // ======================================================
        // ===== LÓGICA DE MODALES (Refactorizado) =====
        // ======================================================
        ...
        /**
         * (NUEVO) Función para el botón "Asignarme a mí"
         */
        async function asignarmeAmi() {
            ...
            // No hacer nada si ya está asignado a este usuario
        ...
        /**
         * (Refactorizado) Función para el botón "Guardar Cambios" (Manual)
         */
        ...
        // --- Modales de Multi-Asignación (Refactorizado) ---
        ...
        /**
         * (Refactorizado) Guarda múltiples asignaciones usando un Batch de Firestore
         */
        async function saveMultiAssignment() {
            ...
            try {
                ...
                // ¡Usar un Batch de Firestore para una sola operación!
                const batch = db_firestore.batch();
                const user = usuarioActual.displayName || usuarioActual.email;
        ...
        // ======================================================
        // ===== LÓGICA DE UI (Refactorizada y Original) =====
        // ======================================================
                ...
                listDiv.innerHTML += `
                    <div class="designer-item flex justify-between items-center p-3 bg-white border border-gray-200 rounded-lg">
                        ...
                        <button class="btn-delete-designer font-medium py-1 px-3 rounded-lg text-xs transition-colors shadow-sm bg-red-100 text-red-700 hover:bg-red-200 flex items-center gap-1" 
                                data-name="${safeName}"
                                data-id="${escapeHTML(docId)}"> <!-- (NUEVO) Añadir ID de doc -->
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4">
                              <path fill-rule="evenodd" d="M8.75 1A2.75 2.75 0 0 0 6 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 1 0 .23 1.482l.149-.022.841 10.518A2.75 2.75 0 0 0 7.596 19h4.807a2.75 2.75 0 0 0 2.742-2.53l.841-10.518.149.022a.75.75 0 1 0 .23-1.482A41.03 41.03 0 0 0 14 4.193v-.443A2.75 2.75 0 0 0 11.25 1h-2.5ZM10 4c.84 0 1.673.025 2.5.075V3.75a1.25 1.25 0 0 0-1.25-1.25h-2.5A1.25 1.25 0 0 0 7.5 3.75v.325C8.327 4.025 9.16 4 10 4ZM8.58 7.72a.75.75 0 0 0-1.5.06l.3 7.5a.75.75 0 1 0 1.5-.06l-.3-7.5Zm4.34.06a.75.75 0 1 0-1.5-.06l-.3 7.5a.75.75 0 1 0 1.5.06l.3-7.5Z" clip-rule="evenodd" />
                            </svg>
                            Eliminar
        ...
        // --- Lógica de Selección Múltiple (Original) ---
        ...
        // --- Lógica de Plan Semanal (Refactorizado) ---
        async function addSelectedToWorkPlan() {
            ...
            for (const orderId of selectedOrders) {
                ...
                try {
                    // Llama a la función de Firestore
            ...
            clearSelection();
            // onSnapshot se encargará de actualizar
        }
        ...
        // --- Lógica de UI y Actualización (Original, pero datos vienen de `allOrders` fusionado) ---
        function populateFilterDropdowns() {
            ...
            updateAllDesignerDropdowns(); // Asegurar que todo esté sincronizado
        }
        ...
        /**
         * (Refactorizado) Esta función ahora solo actualiza la UI.
         * La fusión de datos ocurre en `mergeYActualizar`.
         */
        async function updateDashboard() {
            if (!isExcelLoaded) return; // No hacer nada si el Excel no está cargado
            
            if (needsRecalculation) {
                recalculateChildPieces(); // Sincrónico, lee del mapa
            }
        ...
        // --- Lógica de Reportes y Exportación (Original) ---
        // 'resetApp' ahora solo borra el Excel, no la conexión
        function resetApp() {
        ...
        // --- Lógica de Reporte Semanal (Original, ahora lee de 'allOrders' fusionado) ---
        ...
        async function loadUrgentOrdersToPlan() {
            ...
            // onSnapshot refrescará la vista
        }
        ...
        // --- Lógica de Métricas (Original, ahora lee de 'allOrders' fusionado) ---
        ...
        // --- Lógica de Métricas de Diseñador (Original, pero lee de 'allOrders' fusionado) ---
        ...
        // --- Lógica de Comparación (Original, pero lee de 'allOrders' fusionado) ---
